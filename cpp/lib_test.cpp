...
void checkEngineSpecs(nvinfer1::ICudaEngine* engine) {
    ...
    for (int i = 0; i < numBindings; ++i) {
        ...
        std::cout << std::endl;
        std::cout << "  Data type: " << (dtype == nvinfer1::DataType::kFLOAT ? "FLOAT" : "Other") << std::endl;
    }
}

std::vector<float> loadImage() {
    fs::create_directory("results");
    std::string image_path = "images/dark_frame_0138.jpg";
    ...
    cv::Mat normalizedImg(resized_img.rows, resized_img.cols, CV_32FC3, og_image.data());
...
int main() {
    ...
    size_t inputSize = 196608; // ->create func for both of this with the information above
    ...
    status = cudaMemcpy(inputDevice, og_image.data(), inputSize, cudaMemcpyHostToDevice);
    if (status != cudaSuccess) {
        throw std::runtime_error("Failed to copy data from host to device for input.");
    }
    ...
    // a partir daqui, o modelo ja correu, vamos retirar a informacao do output para uma imagem

   // Copy data from device to host for the output
    std::vector<float> outputHostData(outputSize / sizeof(float));
    status = cudaMemcpy(outputHostData.data(), outputDevice, outputSize, cudaMemcpyDeviceToHost);
    if (status != cudaSuccess) {
        throw std::runtime_error("Failed to copy data from device to host for output.");
    }
    ...
    for (int i = 0; i < outputHeight * outputWidth; ++i) {
        outputMask[i] = outputHostData[i];  // Assuming outputHostData is flattened
    }
    
    // Apply threshold to the mask (qualquer coisa acima do threshold e' mask, abaixo e' background)
    float threshold = 0.5f;  // You can experiment with this value -> experimentei, resultados muito semelhantes
    for (int i = 0; i < outputHeight * outputWidth; ++i) {
    ...
    cv::Mat laneMask(outputHeight, outputWidth, CV_32F, outputMask.data()); // CV_32F for float
    
    // Normalize the mask to [0, 255] (since OpenCV expects 8-bit or 32-bit)
    cv::Mat displayMask;
    laneMask.convertTo(displayMask, CV_8U, 255.0);  // Convert to 8-bit, 255 for white
